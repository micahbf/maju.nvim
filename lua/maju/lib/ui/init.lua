...
local Renderer = require("maju.lib.ui.renderer")
...
---@return Component|nil
function Ui:find_component_by_change_id(change_id)
  return self.node_index:find_by_change_id(change_id)
end
...
--- returns change_ids in selection in a constant order
---@return string[]
function Ui:get_changes_in_selection()
  ...
  for i = start, stop do
    ...
    if component then
      table.insert(changes, 1, component.options.change_id)
    end
  end

  return util.deduplicate(changes)
end

--- returns change_ids in selection ordered according to the direction of the selection the user has made
---@return string[]
function Ui:get_ordered_changes_in_selection()
  ...
  for i = start, stop, increment do
    ...
    if component then
      table.insert(changes, component.options.change_id)
    end
  end
...
---@return string|nil
function Ui:get_change_under_cursor()
  local cursor = vim.api.nvim_win_get_cursor(0)
  local component = self:_find_component_by_index(cursor[1], function(node)
    return node.options.change_id ~= nil
  end)

  return component and component.options.change_id
end
...
function Ui:resolve_cursor_location(cursor)
  ...
  local section = util.find(self.item_index, function(s)
  ...
  local file = util.find(section.items, function(f)
  ...
  local hunk = util.find(file.diff.hunks, function(h)
