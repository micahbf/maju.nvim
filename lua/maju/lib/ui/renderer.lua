...
function RendererIndex:find_by_change_id(change_id)
  return self.change_id_index[change_id]
end
...
function RendererIndex:add_item(item, first, last)
  ...
  if item.change_id then
    self.change_id_index[item.change_id] = item
  end
end
